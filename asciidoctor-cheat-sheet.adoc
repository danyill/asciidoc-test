...
// QUESTION: should the headings be links to more detailed information?
...
It's not covered here but there are also the {asciidoctor-user-manual-icon}[icon] and the {asciidoctor-user-manual-audio}[audio] macros.
...
|width
|Second positional, horizontal dimension in % or pixels, e.g. `width=400`
...
|align
|`[left\|center\|right]` -- for block images only; Align and float are mutually exclusive
...
For using a YouTube or Vimeo, use the identifier in the macro target and the service as the first positional attribute.
...
|options 
|`[autoplay\|loop\|modest\|nocontrols\|nofullscreen]` `modest` reduces YouTube branding.
...
Lots of editors support Asciidoc and usually offer at least syntax highlighting. Some offer advanced features -- outline views, preview, synchronised scrolling and more!
...
* Asciidoctor is super-extensible allowing overrides to implement custom macros, special types of blocks and even completely different converts. {uri-asciidoctor-user-manual-extensions}[Check it out!]
...
* {uri-revealjs-asciidoctor}[reveal.js] -- Want to make a presentation, check out how to use asciidoctor with the {uri-revealjs}[popular presentation framework]
